...
(
    ...
    constraint pk_image_history primary key (id, revision_id),
...
(
    ...
    revision_id   bigint                   not null,
    revision_type smallint                 not null,
    constraint pk_exercise_images_history primary key (exercise_id, image_id, revision_id),
    constraint fk_exercise_images_hist_revinfo
        foreign key (revision_id)
            references life_manager_history.revinfo (rev)
            on delete cascade
);